...
import { appwriteConfig, databases, ID, Permission, Query, Role } from '../lib/appwrite'
...
async function getLastPagesEndForBook(userId: string, bookId: string) {
  const { databaseId, logsCollectionId } = appwriteConfig
  if (!databaseId || !logsCollectionId) return undefined
  ...
  for (const l of res.documents) {
    if (typeof l.pagesEnd === 'number') return l.pagesEnd
  }
  return undefined
}
...
export default function CreateReadingLogPage() {
  ...
  const [pagesStart, setPagesStart] = createSignal('')
  const [pagesEnd, setPagesEnd] = createSignal('')
  const [note, setNote] = createSignal('')

  const [lastEnd] = createResource(
    () => {
      const userId = session.user()?.$id
      const id = bookId()
      return userId && id ? { userId, id } : undefined
    },
    async (v) => (v ? getLastPagesEndForBook(v.userId, v.id) : undefined)
  )

  createEffect(() => {
    const v = lastEnd()
    if (typeof v !== 'number') return
    if (pagesStart().trim()) return
    setPagesStart(String(v))
  })
    ...
    try {
      ...
      await databases.createDocument<LogDoc>(
        ...
        ID.unique(),
        {
          ...
          minutes: toIntOrUndefined(minutes()),
          pagesStart: toIntOrUndefined(pagesStart()),
          pagesEnd: toIntOrUndefined(pagesEnd()),
      ...
      navigate('/reading-logs')
    } catch (err: any) {
      setError(err?.message ?? 'Failed to create reading log')
    } finally {
      setBusy(false)
    }
  }
    ...
    <div class="bg-mainBg font-sans text-textPrimary h-screen flex overflow-hidden">
      ...
      <main class="flex-1 overflow-y-auto" data-purpose="dashboard-content">
        <header class="flex justify-between items-center py-4 px-4 md:py-6 md:px-10 sticky top-0 z-40 border-b border-gray-100 bg-mainBg bg-opacity-90 backdrop-blur-md backdrop-saturate-150">
          <div>
            <h2 class="text-2xl font-bold text-gray-900">Create Reading Log</h2>
          ...
          <div class="max-w-3xl">
            ...
            <form class="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-6" onSubmit={submit}>
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">Pages start</label>
                  <input
                    class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-black focus:border-black block w-full p-2.5"
                    inputmode="numeric"
                    type="text"
                    placeholder={typeof lastEnd() === 'number' ? String(lastEnd()) : undefined}
                    value={pagesStart()}
                    onInput={(e) => setPagesStart(e.currentTarget.value)}
                  />
                  <Show when={typeof lastEnd() === 'number'}>
                    <p class="text-xs text-gray-500 mt-1">Pre-filled from your last session end page.</p>
                  </Show>
                </div>
                <div>
                  <label class="block text-sm font-medium text-gray-700 mb-2">Pages end</label>
                  <input
                    class="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-black focus:border-black block w-full p-2.5"
                    inputmode="numeric"
                    type="text"
                    value={pagesEnd()}
                    onInput={(e) => setPagesEnd(e.currentTarget.value)}
                  />
                </div>
                <div>
              ...
              <div class="flex items-center justify-end gap-3">
                ...
                >
                  ...
                  <span>{busy() ? 'Creating…' : 'Create Log'}</span>
                </button>
              </div>
            </form>
          </div>
